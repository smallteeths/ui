...
export default Component.extend(NewOrEdit, ChildHook, {
  ...
  init() {
    ...
    if (this.launchConfig && !get(this, 'launchConfig.envFrom')) {
    ...
    if (!this.isSidekick && !get(service, 'scheduling.node') && !get(service, 'scheduling.podAffinity') && !get(service, 'scheduling.podAntiAffinity')) {
  ...
  validate() {
    ...
    errors.pushObjects(this.lifecycleErrors || []);
    errors.pushObjects(this.metricsErrors || []);
