  ...
  willSave() {
    ...
    if (get(this, 'pasteOrUpload')) {
      ...
      set(model, 'annotations', {
        ...
        'field.cattle.io/fluentdlogDir': fluentdLogDir,
      })

      if (enableMultiLineFilter
          && (!multiLineStartRegexp || multiLineStartRegexp.trim() === '')
          && (!multiLineEndRegexp || multiLineEndRegexp.trim() === '')
      ) {
        errors.pushObject(intl.t('loggingPage.additional.enableMultiLineFilter.required'))
      }

      if (!fluentdLogDir) {
        errors.pushObject(`${ intl.t('loggingPage.additional.fluentdLogDir.label') }${ intl.t('generic.isRequired') }`)
      }
    } else {
      ...
      setProperties(model, {
        ...
        enableExceptionStackMatch: get(model, `${ targetType }.enableExceptionStackMatch`),
      ...
      set(model, 'annotations', {
        ...(get(model, 'annotations') || {}),
        'field.cattle.io/fluentdlogDir': get(model, `${ targetType }.fluentdLogDir`),
      })

      const {
        enableMultiLineFilter, multiLineStartRegexp, multiLineEndRegexp
      } = model
      ...
      if (!get(model, `${ targetType }.fluentdLogDir`)) {
        errors.pushObject(`${ intl.t('loggingPage.additional.fluentdLogDir.label') }${ intl.t('generic.isRequired') }`)
      }
