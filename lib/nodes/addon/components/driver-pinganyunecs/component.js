...
export default Component.extend(Driver, {
  ...
  actions: {
    showNetworkConfig(cb) {
      ...
      all(promises).then(([regions, osTypes]) => {
        ...
        const osTypeChanged = this.setOptionsDefaultValue('config.osTypeId', 'osTypes');
    ...
    showInstanceConfig(cb) {
      ...
      if (diskType) {
        ...
        // }
        const size = parseInt(diskSize, 10);

        if (!/^[0-9]+$/.test(diskSize) || size > 2000 || size < 20 || size !== 0) {
  ...
  regionDidChange: observer('config.region', function() {
    ...
    if (!regionId) {
      ...
      set(this, 'config.productSeriesId', null);
      set(this, 'config.osTypeId', null);
      ...
      set(this, 'config.securityGroupId', null);
    ...
    this.loadNatGateways(regionId).then((nats) => {
      set(this, 'nats', nats);
      const natIdChanged = this.setOptionsDefaultValue('config.natId', 'nats');
  ...
  vpcDidChange: observer('config.vpcId', function() {
    ...
    if (!vpcId) {
      ...
      set(this, 'config.networkId', null);
      set(this, 'config.securityGroupId', null);
      set(this, 'config.securityGroupName', 'rancher-nodes');
  ...
  zoneDidChange: observer('config.zone', function() {
    ...
    this.loadProductSeries(zoneId).then((ps) => {
      ...
      const psChanged = this.setOptionsDefaultValue('config.productSeriesId', 'productSeries');
  ...
  productSeriesDidChange: observer('config.productSeriesId', function() {
    const productSeriesId = get(this, 'config.productSeriesId');
  ...
  networkDidChange: observer('config.networkId', function() {
    ...
    this.loadSecurityGroups(networkId, regionId, vpcId).then((securityGroups) => {
      ...
      const sgIdChanged = this.setOptionsDefaultValue('config.securityGroupId', 'securityGroups', null);

      if (!sgIdChanged) {
        this.securityGroupIdDidChange();
      }
    });
  }),
  natIdDidChange: observer('config.natId', function() {
    const natId = get(this, 'config.natId');
    const nats = get(this, 'nats') || [];
    const currentNat = nats.find((nat) => nat.value === natId)

    if (currentNat) {
      this.loadSnatNames(natId).then((snatNames) => {
        set(this, 'snatNames', snatNames);
        this.setOptionsDefaultValue('config.snatName', 'snatNames');
      });
    } else {
      set(this, 'config.snatName', null);
    }
  }),
  reloadImages: observer('config.useType', 'config.osTypeId', 'config.region', 'config.zone', function() {
    const osTypeId = get(this, 'config.osTypeId');
    const regionId = get(this, 'config.region');
    const zoneId = get(this, 'config.zone');
    const useType = get(this, 'config.useType');
  ...
  securityGroupIdDidChange: observer('config.securityGroupId', function() {
    if (get(this, 'config.securityGroupId')) {
      set(this, 'config.securityGroupName', null);
    } else {
      set(this, 'config.securityGroupName', 'rancher-nodes');
    }
  }),
  ...
  currentSecurityGroup: computed('securityGroups', 'config.securityGroupId', function() {
  ...
  currentSnatName: computed('snatNames', 'config.snatName', function() {
    const snatNames = get(this, 'snatNames') || [];
    const snatName = get(this, 'config.snatName');
    const found = snatNames.find((n) => n.value === snatName);
  ...
  loadSnatNames(natId) {
    return this.apiRequest('ListSnatRule', {
      natId,
      status: 'Available',
    }).then((resp) => {
      const snatNames = resp.NatBeanList.map((snat) => {
        return {
          value: snat.Name,
          label: snat.Name,
          raw:   snat,
  ...
  setOptionsDefaultValue(valueKey, optionsKey, defaultValue) {
    const value = get(this, valueKey);
    let valueChanged = true;
    const firstValue = get(this, `${ optionsKey }.firstObject.value`);
    ...
    if (value) {
      ...
      if (!found) {
        if (arguments.length === 2) {
          set(this, valueKey, firstValue);
        } else {
          set(this, valueKey, defaultValue);
        }
      } else {
        valueChanged = false;
      }
