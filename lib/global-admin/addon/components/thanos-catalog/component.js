...
const S3_BUCKET_NAME = 'thanos.objectConfig.config.bucket'
const S3_ENDPOINT = 'thanos.objectConfig.config.endpoint'
const S3_ACCESS_KEY = 'thanos.objectConfig.config.access_key'
const S3_SECRET_KEY = 'thanos.objectConfig.config.secret_key'
const S3_TYPE = 'thanos.objectConfig.type'
...
const HIDDEN_KEYS = ['clusterIds', 'rancherHostname', 'token', 'apiToken', 'ui.service.type', THANOS_COMPACT_ENABLED, THANOS_STORE_ENABLED, S3_TYPE]
    ...
    save(cb) {
      ...
      if ( this.objectStorageEnabled ) {
        if (!this.config.s3BucketName ) {
          errors.pushObject(requiredError('globalMonitoringPage.store.s3.bucketName.label'))
        }
        if (!this.config.s3Endpoint ) {
          errors.pushObject(requiredError('globalMonitoringPage.store.s3.endpoint.label'))
        }
        if (!this.config.s3AccessKey ) {
          errors.pushObject(requiredError('globalMonitoringPage.store.s3.accessKey.label'))
        }
        if (!this.config.s3AccessKey ) {
          errors.pushObject(requiredError('globalMonitoringPage.store.s3.secretKey.label'))
        }
      }
