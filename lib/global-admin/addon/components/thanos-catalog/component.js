...
const HIDDEN_KEYS = ['clusterIds', 'rancherHost', 'token', 'apiToken', 'ui.service.type', THANOS_COMPACT_ENABLED, THANOS_STORE_ENABLED]
...
export default Component.extend(CrudCatalog, ReservationCheck, CatalogUpgrade, {
  ...
  initAnswers() {
    ...
    const generalKeys = Object.keys(answers).filter((key) => (this.objectStorageEnabled && objectStorageAnswersArrary.indexOf(key) === -1) && OBJECT_STORAGE_PROVIDER_TYPE !== key);
