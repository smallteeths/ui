...
export default Component.extend(ViewNewEdit, {
  ...
  config: computed('cloudCredentialType', 'model.{aliyunecscredentialConfig,amazonec2credentialConfig,azurecredentialConfig,digitaloceancredentialConfig,googlecredentialConfig,linodecredentialConfig,ocicredentialConfig,pnapcredentialConfig,vmwarevspherecredentialConfig}', function() {
    const { model }   = this;
  ...
  willSave() {
    ...
    if (keysThatWeCanValidate.includes(cloudCredentialType)) {
      ...
      if (cloudCredentialType === 'alibaba') {
        ...
        let endpoint = 'ecs.aliyuncs.com';

        endpoint = `/meta/proxy/${  endpoint.replace('//', '/') }`;
