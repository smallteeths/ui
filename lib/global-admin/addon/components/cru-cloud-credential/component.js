...
import { REGIONS as HUAWEI_REGIONS } from 'shared/utils/huawei';
...
const CRED_CONFIG_CHOICES = [
  {
    ...
    displayName:       'Alibaba',
    driver:            'aliyun',
    configField:       'aliyunecscredentialConfig',
    kontainerDriverId: 'aliyunkubernetescontainerservice',
    operatorDriverId:  'ackoperatorsetting',
  },
  {
    name:              'amazon',
    displayName:       'Amazon',
  ...
  },
  {
    ...
    displayName:       'Tencent',
    driver:            'tencent',
    configField:       'tkecredentialConfig',
    kontainerDriverId: 'tencentkubernetesengine'
  },
  {
    name:              'vmware',
  ...
  },
  {
    ...
    displayName:       'Huawei',
    driver:            'huawei',
    configField:       'huaweicredentialConfig',
    operatorDriverId:  'cceoperatorsetting'
  },
  {
    name:              'tencent',
    displayName:       'Tencent',
    driver:            'tke',
    configField:       'tkecredentialConfig',
    operatorDriverId:  'tkeoperatorsetting'
  },
]
...
const HUAWEI_REGION_CHOICES = getHuaweiRegionChoices(HUAWEI_REGIONS);
...
export default Component.extend(ViewNewEdit, {
  ...
  huaweiRegionChoices:       HUAWEI_REGION_CHOICES,
  ...
  config: computed('cloudCredentialType', 'model.{aliyunecscredentialConfig,amazonec2credentialConfig,azurecredentialConfig,digitaloceancredentialConfig,googlecredentialConfig,harvestercredentialConfig,linodecredentialConfig,ocicredentialConfig,pnapcredentialConfig,vmwarevspherecredentialConfig,tkecredentialConfig,huaweicredentialConfig}', function() {
    const { model }   = this;
  ...
  configChoices: computed('driverName', 'allNodeDrivers', 'allKontainerDrivers', 'allOperatorDrivers', function() {
    if (get(this, 'driverName')) {
      ...
      if (get(this, 'allNodeDrivers') && get(this, 'allKontainerDrivers') && get(this, 'allOperatorDrivers')) {
        return (CRED_CONFIG_CHOICES.filter((N) => {
          const isActive = (get(this, 'allNodeDrivers') || []).find((O) => O.id === N.driver && O.active === true) ||  (get(this, 'allKontainerDrivers') || []).find((O) => O.id === N.kontainerDriverId && O.active === true) ||  (get(this, 'allOperatorDrivers') || []).find((O) => O.id === N.operatorDriverId && O.active === true);
  ...
  willSave() {
    ...
    const { cloudCredentialType }      = this;
    const keysThatWeCanValidate = ['alibaba', 'amazon', 'digitalOcean', 'linode', 'oci', 'google', 'azure', 'tencent', 'huawei'];
    const auth                  = {
      type:  'validate',
      token: null,
    };
    ...
    if (keysThatWeCanValidate.includes(cloudCredentialType)) {
      ...
      if (cloudCredentialType === 'alibaba') {
        const location = window.location;
        const endpoint = `${ location.origin }/meta/proxy/ecs.aliyuncs.com`;
        let authConfig = {
        ...
        let ecs        = new ALY.ECS(authConfig);

        return new Promise((resolve, reject) => {
          ecs.describeRegions({}, (err) => {
            if ( err ) {
              reject(err);
            }
      ...
      if (cloudCredentialType === 'tencent') {
        return get(this, 'globalStore').rawRequest({
          url:    '/meta/tke/tkeCheckCredentials',
          method: 'POST',
          data:   {
            secretId:  get(this, 'config.accessKeyId'),
            secretKey: get(this, 'config.accessKeySecret'),
          },
        }).then(() => {
          set(this, 'validatingKeys', false);
      ...
      if (cloudCredentialType === 'huawei') {
        const regionID =  get(this, 'config.regionID');

        if (!regionID){
          const defaultRegionID = get(this, 'huaweiRegionChoices.firstObject.value');

          set(this, 'config.regionID', defaultRegionID)
        }
        ...
        return get(this, 'globalStore').rawRequest({
          ...
          set(this, 'validatingKeys', false);
          set(this, 'huaWeiRegionID', get(this, 'config.regionID'));
  ...
  doneSaving(neu) {
    ...
    } else if (driverName === 'huawei') {
      set(neu, this.getConfigField(), {});
      if (neu) {
        if (neu.huaweicredentialConfig) {
          neu.huaweicredentialConfig.regionID = get(this, 'huaWeiRegionID');
        }
        neu.regionID = get(this, 'huaWeiRegionID');
      }
      set(this, 'huaWeiRegionID', null)
    } else {
  ...
  feetchOperatorDriver() {
    get(this, 'globalStore').rawRequest({ url: '/v3/operatorsettings' }).then((res) => {
      const allOperatorDrivers = get(res, 'body.data');

      set(this, 'allOperatorDrivers', allOperatorDrivers);
    });
  },
