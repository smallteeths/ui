...
export default Component.extend(ViewNewEdit, {
  ...
  config: computed('cloudCredentialType', 'model.{aliyunecscredentialConfig,amazonec2credentialConfig,azurecredentialConfig,digitaloceancredentialConfig,googlecredentialConfig,harvestercredentialConfig,linodecredentialConfig,ocicredentialConfig,pnapcredentialConfig,vmwarevspherecredentialConfig}', function() {
    const { model }   = this;
  ...
  willSave() {
    ...
    const { cloudCredentialType }      = this;
    const keysThatWeCanValidate = ['alibaba', 'amazon', 'digitalOcean', 'linode', 'oci', 'google', 'azure'];
    const auth                  = {
      type:  'validate',
      token: null,
    };
