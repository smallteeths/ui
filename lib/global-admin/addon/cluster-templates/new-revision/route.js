    ...
    return this.globalStore.find('clustertemplate', params.template_id).then((template) => {
      return this.globalStore.findAll('clustertemplaterevision').then((revisions) => {
        ...
        if (template.defaultRevisionId) {
          ...
          // set network plugin default value
          const plugin = get(tempRevision, 'clusterConfig.rancherKubernetesEngineConfig.network.plugin');
          const options = get(tempRevision, 'clusterConfig.rancherKubernetesEngineConfig.network.options');

          if (plugin === 'none' && options && options.pandariaExtraPluginName) {
            set(tempRevision, 'clusterConfig.rancherKubernetesEngineConfig.network.plugin', options.pandariaExtraPluginName);
          }
