...
export default Route.extend({
  ...
  actions: {
    error(err, transition) {
      const status = parseInt(err && err.status, 10);

      if (403 === status) {
        transition.abort();
        this.growl.fromError(this.intl.t('globalDnsPage.entriesPage.access.noAccessAddProvider'), err.body.message);
  ...
  initConfig(configType = 'route53') {
    if ( configType === 'route53' ) {
    ...
    } else if ( configType === 'f5bigip' ) {
      let f5bigipProviderConfig = this.globalStore.createRecord({ type: 'f5bigipProviderConfig' })

      if (!f5bigipProviderConfig.f5BigipDeviceIPs) {
        f5bigipProviderConfig.f5BigipDeviceIPs = [{
          name:        '',
          deviceName:  '',
          translation: '',
        }]
      ...
      return this.globalStore.createRecord({
        type:                  'globaldnsprovider',
        providerName:          'f5bigip',
        provider:              'f5bigip',
        f5bigipProviderConfig,
      });
    }
  },
