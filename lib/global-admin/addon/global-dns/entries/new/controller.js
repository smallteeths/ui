...
export default Controller.extend(ViewNewEdit, {
  ...
  validationErrors() {
    ...
    if (!this.primaryResource?.fqdn) {
      errors.push(intl.t('validation.required', { key: 'FQDN' }));
    }
    if (!this.primaryResource?.providerId) {
      errors.push(intl.t('validation.required', { key: 'Provider' }));
    }
