...
const poolLbMode = ['global-availability', 'ratio', 'round-robin', 'topology']
const fallbackMode = ['completion-rate', 'cpu', 'drop-packet', 'fallback-ip', 'fewest-hops', 'global-availability', 'kilobytes-per-second', 'least-connections', 'lowest-round-trip-time ', 'none', 'packet-rate', 'quality-of-service', 'ratio', 'return-to-dns', 'round-robin', 'tatic-persistence', 'topology', 'virtual-server-capacity', 'virtual-server-score']
const loadBalancingMode = ['completion-rate', 'cpu', 'drop-packet', 'fallback-ip', 'fewest-hops', 'global-availability', 'kilobytes-per-second', 'least-connections', 'lowest-round-trip-time', 'packet-rate', 'quality-of-service', 'ratio', 'return-to-dns', 'round-robin', 'static-persistence', 'topology', 'virtual-server-capacity', 'virtual-server-score']
const alternateMode = ['droppacket', 'fallback-ip', 'global-availability', 'none', 'packet-rate', 'ratio', 'return-to-dns', 'round-robin', 'static-persistence', 'topology', 'virtual-server-capacity', 'virtual-server-score']
const poolLbModeOptions = formatOption(poolLbMode)
const fallbackModeOptions = formatOption(fallbackMode)
const loadBalancingModeOptions = formatOption(loadBalancingMode)
const alternateModeOptions = formatOption(alternateMode)
...
export default Controller.extend(ViewNewEdit, {
  ...
  headers:                   HEADERS,
  // F5 globaldns load balancing mode
  poolLbModeValue:           'round-robin',
  poolLbModeOptions,
  fallbackModeValue:         'return-to-dns',
  fallbackModeOptions,
  loadBalancingModeValue:    'round-robin',
  loadBalancingModeOptions,
  alternateModeValue:        'round-robin',
  ...
  isF5BigipProvider: computed('model.providers', 'primaryResource.providerId', function() {
    let { providerId } = this.primaryResource;
    let { providers }  = this.model;
  ...
  setF5bigipAnnotation() {
    if (get(this, 'isF5BigipProvider')) {
      let f5bigipAnnotation = { 'f5.cattle.io/globaldns': true }

      get(this, 'annoConfig').forEach((item) => {
        f5bigipAnnotation[item.key] = get(this, item.value)
      })
      if (get(this, 'primaryResource.annotations')) {
        set(this, 'primaryResource.annotations', Object.assign(get(this, 'primaryResource.annotations'), f5bigipAnnotation))
      } else {
        set(this, 'primaryResource.annotations', f5bigipAnnotation)
      }
    } else {
      if (get(this, 'primaryResource.annotations')) {
        if (get(this, 'primaryResource.annotations')['f5.cattle.io/globaldns']) {
          delete get(this, 'primaryResource.annotations')['f5.cattle.io/globaldns']
        }
        get(this, 'annoConfig').forEach((item) => {
          if (get(this, 'primaryResource.annotations')[item.key]) {
            delete get(this, 'primaryResource.annotations')[item.key]
          }
        })
      }
    }
  ...
  willSave() {
    this.setF5bigipAnnotation();
    // Pandaria. If the key is not Assigning values, Do not passed to the backend.
    this.primaryResource?.fqdn === '' && delete this.primaryResource?.fqdn
    this.primaryResource?.providerId === null && delete this.primaryResource?.providerId
