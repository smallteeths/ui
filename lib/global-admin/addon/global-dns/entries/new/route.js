...
export default Route.extend({
  ...
  setupController(controller, model) {
    ...
    let mode          = 'new';
    // Pandaria: Init anno for f5bigip
    let f5bigipAnnotationValueObject = {
    ...
    // Pandaria: anno key for bigip load balancing
    const annoConfig = [
      {
        key:   'external-dns.alpha.kubernetes.io/bigip-pool-lb-mode',
        value: 'poolLbModeValue'
      },
      {
        key:   'external-dns.alpha.kubernetes.io/bigip-pool-member-load-balancing-mode',
        value: 'loadBalancingModeValue'
      },
      {
        key:   'external-dns.alpha.kubernetes.io/bigip-pool-member-alternate-mode',
        value: 'alternateModeValue'
      },
      {
        key:   'external-dns.alpha.kubernetes.io/bigip-pool-member-fallback-mode',
    ...
    if (get(model, 'globaldns.id')) {
      ...
      let anno = get(model, 'globaldns.annotations');

      annoConfig.forEach((item) => {
        if (anno && anno[item.key]) {
          f5bigipAnnotationValueObject[item.value] = anno[item.key]
        }
      })
    }
