...
export default Route.extend({
  ...
  model() {
    ...
    const reqs = {
      ...
      operatorsettings:         gs.rawRequest({ url: '/v3/operatorsettings' }).then((res) => res?.body?.data || []).catch(() => ([])),
    };
  ...
  afterModel(model) {
    ...
    let externalOperators    = operatorsettings.filter((o) => o.url !== '' && o.state === 'active')
    let promises = {};
