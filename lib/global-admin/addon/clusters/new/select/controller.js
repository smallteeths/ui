...
const OPERATOR_DRIEVERS = { ackoperatorsetting: 'aliyun' };
...
export default Controller.extend({
  ...
  kontainerDrivers: computed('model.kontainerDrivers.@each.{id,state}', function() {
    const build_in_ui = ['tencentkubernetesengine', 'oraclecontainerengine', 'linodekubernetesengine'];
  ...
  providerChoices: computed('app.proxyEndpoint', 'intl.locale', 'kontainerDrivers.[]', 'model.nodeDrivers.{id,state}', 'schemaReloaded', 'externalOperator', function() {
    const { kontainerDrivers, intl  } = this;
    const { builtIn, custom } = kontainerDrivers;
    const externalOperator = this.externalOperator;
    ...
    if (externalOperator.length > 0){
      const operatorDriver = OPERATOR_DRIEVERS;

      externalOperator.forEach((o) => {
        let {
          displayName, name, name: driver, genericIcon
        }           = o;

        if (operatorDriver[o.name]){
          const kontainerDriver = out.findBy('name', operatorDriver[o.name]);

          if (kontainerDriver){
            out.removeObject(kontainerDriver);
          }
        }
  ...
  showImportAck: computed('providerGroups.cloudGroup', function() {
    return get(this, 'providerGroups.cloudGroup').findBy('driver', 'ackoperatorsetting');
  }),
  showImportTke: computed('providerGroups.cloudGroup', function() {
    return get(this, 'providerGroups.cloudGroup').findBy('driver', 'tkeoperatorsetting');
  }),
  showImportCce: computed('providerGroups.cloudGroup', function() {
    return get(this, 'providerGroups.cloudGroup').findBy('driver', 'cceoperatorsetting');
  }),

  externalOperator: computed('model.operatorsettings.@each.{id,state}', function(){
    const eOperator = get(this, 'model.operatorsettings') || [];

    return eOperator.filter((o) => o.state === 'active')
  }),
