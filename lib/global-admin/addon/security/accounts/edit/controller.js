...
export default Controller.extend(NewOrEdit, {
  ...
  willSave() {
    ...
    if (get(this, 'passwordStrength') < 2 && get(this, 'password.length') > 0 && (get(this, 'password') || '').trim().length > 0) {
      errors.push(get(this, 'intl').t('accountsPage.new.errors.strengthError'));
    }
  ...
  didSave() {
    ...
    if (get(this, 'password.length') > 0) {
      promise = get(this, 'primaryResource').doAction('setpassword', { newPassword: get(this, 'access').encryptPassword(get(this, 'password').trim()) });
    }
