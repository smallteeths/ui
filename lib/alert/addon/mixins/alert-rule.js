...
export default Mixin.create({
  ...
  init() {
    ...
    rules.forEach((rule, index) => {
      ...
      if (rule && get(rule, 'metricRule.description') === 'Template Rules') {
        set(rule, 'metricRule.templateRule', get(rule, 'metricRule.expression'))
        set(rule, 'metricRule.expression', '')
        set(rule, '_targetType', 'templateRules')
        set(rules, index, rule)
      }
  ...
  monitoringEnabled: computed('scope.currentCluster.enableClusterMonitoring', function() {
  ...
  beforeSaveClusterAlert(alertRule, group) {
    ...
    case 'templateRules':
      ...
      setProperties(get(clone, 'metricRule'), {
        ...
        expression:  get(clone, 'metricRule.templateRule'),
        commonRule:  null,
        unit:        null
      })
      break;
