...
export function convertToLimit(key, value) {
  ...
  case 'limitsCpu':
  case 'requestsCpu':
    return convertToMillis(value).toString();
  case 'limitsMemory':
  case 'requestsMemory':
    return (parseSi(value, 1024) / 1048576).toString();
  case 'requestsStorage':
  case 'requestsStorageClassStorage':
    return (parseSi(value) / (1024 ** 3)).toString();
  default:
    return value;
  }
}
...
export function quotaWithUnits(label, value, readable = false) {
  ...
  let storageNotation = readable ? 'GiB' : 'Gi';
