...
export function keysToDecamelize(obj, ignore = [], drop = [], customMap) {
  ...
  while (n--) {
    const key      = keys[n];
    const titleKey = customMap && customMap[key] ? customMap[key] : decamelize(key);
...
const UNDER_SCORE_PARAMS = ['.rancher_kubernetes_engine_config.network.options', '.rancher_kubernetes_engine_config.dns.options', '.rancher_kubernetes_engine_config.ingress.extra_args'];
const KUBE_SERVICES = ['.rancher_kubernetes_engine_config.services'];

export function keysToCamel(obj, path = '', customMap) {
  if ( !typeof (obj) === 'object' || typeof (obj) === 'string' || typeof (obj) === 'number' || typeof (obj) === 'boolean' || isEmpty(obj) ) {
    return obj;
  }
  const keys = Object.keys(obj);
  ...
  while (n--) {
    ...
    if (customMap && customMap[key]) {
      titleKey = customMap[key]
    } else if ( KUBE_SERVICES.indexOf(path) > -1 ) {
