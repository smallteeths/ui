...
var C = {
  CATALOG: {
    ...
    HELM_STABLE_VALUE:     'https://charts.helm.sh/stable',
    HELM_INCUBATOR_KEY:    'helm-incubator',
    HELM_INCUBATOR_VALUE:  'https://charts.helm.sh/incubator',
    ...
    FINOGEEKS_KEY:         'finogeeks',
    FINOGEEKS_VALUE:       'https://github.com/finogeeks/mop-chart',
  ...
  COOKIE: {
    ...
    IGNORE_CRED_CLONE_WORKLOAD: 'R_IGNORE_CRED_CLONE_WORKLOAD'
  },
  ...
  EXT_REFERENCES: {
    CN_FORUM: 'https://forums.rancher.cn',
    ...
    MIRROR:   'http://mirror.cnrancher.com',
  },
  ...
  PROJECT: {
    ...
    TYPE_SHIBBOLETH_GROUP_UID:   'shibboleth_group_uid',
  ...
  SETTING: {
    ...
    SYSTEM_CATALOG:                     KIND_SYSTEM_CATALOG,
...
C.SETTING.ALLOWED = {
  ...
  [C.SETTING.CATTLE_PASSWORD_MIN_LENGTH]:       { kind: 'int' },
  [C.SETTING.PL]:                             {},
  [C.SETTING.UI_ISSUES]:                      {},
  ...
  [C.SETTING.DISABLE_PASSWORD_ENCRYPT]:          { kind: 'boolean', },
  [C.SETTING.DOWNLOAD_FILE_SIZE_LIMIT]:          {
    kind: 'int',
    unit: 'Mi'
  },
  [C.SETTING.ENABLE_LOAD_RESOURCE_BY_NAMESPACE]: { kind: 'boolean', },
  [C.SETTING.PANDARIA_ENABLE_HEALTHCHECK_API]:   { kind: 'boolean', },
  [C.SETTING.RESTRICTED_DEFAULT_ADMIN]:          { kind: 'boolean', },
  [C.SETTING.ROTATE_CERTS_IF_EXPIRING_IN_DAYS]:  { kind: 'int', },
  [C.SETTING.SYSTEM_CATALOG]:                    {
    kind:    'enum',
...
C.ANNOTATIONS_TO_IGNORE_PREFIX = [
  ...
  'k3s.io',
  'f5.pandaria.io/targets',
  'macvlan.pandaria.io/plugin',
]
...
C.SUPPORT_POWER_API_TYPES = ['user', 'node', 'project', 'projectRoleTemplateBinding', 'secret', 'workload', 'pod', 'ingress', 'configmap', 'namespacedsecret', 'persistentVolumeClaim', 'service'];
...
C.COMMON_RULES = [
  ...
  },
  {
    ...
    value:          '(sum(rate(container_cpu_usage_seconds_total[5m])) by (pod_name, container_name) / sum(label_join(label_join(kube_pod_container_resource_limits_cpu_cores,"pod_name", "", "pod"),"container_name", "", "container")) by (pod_name, container_name)) * 100',
    scope:          'project',
    comparison:     C.ALERTING_COMPARISON.GREATER_OR_EQUAL,
    thresholdValue: 100,
    unit:           'percent'
