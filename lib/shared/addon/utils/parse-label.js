...
export function validateIdentifierCustom(str, displayKey, intl, errors = [], others = {}) {
  let middle_only = others.specials || MIDDLE_ONLY
  let validChars = others.validChars || `A-Za-z0-9${ middle_only.join('') }`

  validateChars(str, { validChars }, displayKey, intl, errors);
  ...
  const min = others.min === undefined ? 0 : others.min;
  const max = others.max || 63;

  if ( str.length < min ) {
    str.length === 0 ? errors.push(intl.t(`validation.required`, { key: displayKey })) : errors.push(intl.t(`validation.stringLength.min`, {
      key:   displayKey,
      count: min
    }))
  } else if ( str.length > max ) {
    errors.push(intl.t(`validation.stringLength.max`, {
      key:   displayKey,
      count: max
    }));
  }
