...
// pandaria
export function addTooltipPandaria(options) {
  let el = options.el
  let tooltip = null
  ...
  if (el.getElementsByTagName('div').length > 0) {
    ...
    tooltip = select(el).select('.percent-dashboard-quota-tooltip');
  } else {
    tooltip = select(el).append('div')
      .attr('class', 'hover-label')
      .attr('class', 'percent-dashboard-quota-tooltip')
      .style('opacity', 0)
      .style('z-index', 100);
  }
...
// Pandaria Binding events
export function bindingEvents(el, options, labelType) {
  let tooltipQuota = null;

  el.attr('cursor', 'pointer')
  el.on('mouseover', () => {
    tooltipQuota = addTooltipPandaria(options)

    tooltipQuota.transition()
      .duration(200)
      .style('opacity', .9);
    tooltipQuota.html(`<div style="padding: 0px 8px;">${ labelType === 'subtitle' ? options.tooltipText : options.LiveTitleTooltip }</div>`)
      .style('left', `${ (el.attr('x')) - (parseInt(tooltipQuota.style('width'), 10) / 2) }px`)
      .style('top', `${ (el.attr('y')) - 8 - parseInt(tooltipQuota.style('height'), 10) }px`)
      .style('z-index', 100);
  }).on('mouseout', () => {
    options.timer = setTimeout(() => {
      let tooltipQuota = select(options.el).select('.percent-dashboard-quota-tooltip');

      tooltipQuota && tooltipQuota.remove()
    }, 500)
  });
}
...
function addLabels(svg, options, width, height, fontSize, mode) {
  ...
  return {
    ...
    subtitleLabel: addText(options.subtitle, svg, width / 2, getSubtitleLabelY(height, fontSize, mode), fontSize / 3, 'subtitle', mode, options),
  }
}
...
export function addText(text, svg, x, y, fontSize, labelType, bold = 0, options) {
  let p = svg.append('svg:text')
    .attr('x', x)
    .attr('y', y)
    .attr('dy', fontSize / 2)
  ...
  if (options && (options.tooltipText || options.LiveTitleTooltip)) {
    bindingEvents(p, options, labelType)
  }
