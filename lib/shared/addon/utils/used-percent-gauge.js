...
import {
  getConfig, addArc, valueToPoint, addText, calcR, createArc, getRange, addTooltipPandaria
...
export default function initGraph(options) {
  ...
  return {
    ...
    updateLiveLabel(text, tooltipText) {
      let tooltipQuota = null;

      liveLabel.text(text);

      // Pandaria update tooltipText
      liveLabel.on('mouseover', () => {
        tooltipQuota = addTooltipPandaria(options)

        tooltipQuota.transition()
          .duration(200)
          .style('opacity', .9);
        tooltipQuota.html(`<div style="padding: 0px 8px;">${ tooltipText }</div>`)
          .style('left', `${ (liveLabel.attr('x')) - (parseInt(tooltipQuota.style('width'), 10) / 2) }px`)
          .style('top', `${ (liveLabel.attr('y')) - 8 - parseInt(tooltipQuota.style('height'), 10) }px`);
      }).on('mouseout', () => {
...
function addLabels(svg, options, width, height, fontSize) {
  return {
    ...
    subtitleLabel: addText(options.subtitle, svg, width / 2, getSubtitleLabelY(height, fontSize), fontSize / 3, 'subtitle', 0, options),
    liveLabel:     addText(options.liveTitle, svg, width / 2, getLiveLabelY(height, fontSize), fontSize / 3, 'liveLabel', 0, options),
  }
}
