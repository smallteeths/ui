...
export default Service.extend({
  ...
  detect() {
    ...
    return hash({
      ...
      disablePasswordEncrypt: globalStore.request({ url: `settings/${ C.SETTING.DISABLE_PASSWORD_ENCRYPT }` }),
      providers:              globalStore.request({ url: '/v3-public/authProviders' }),
    }).then(({
      providers, pl, footerText, footerURL, firstSetting, disablePasswordEncrypt
    }) => {
      ...
      if ( disablePasswordEncrypt ) {
        this.settings.notifyPropertyChange(C.SETTING.DISABLE_PASSWORD_ENCRYPT);
      }
  ...
  encryptPassword(password) {
    const disabledEncryption = get(this, 'settings.disablePasswordEncrypt');

    if (disabledEncryption === 'true') {
      return password;
    }
