...
export default Service.extend({
  ...
  initLanguage(save = false) {
    ...
    const fromExploreCookie  = this.get('cookies').get(C.COOKIE.LOCALE);// get language from explore UI cookie
    ...
    } else if ( window.self !== window.top && fromExploreCookie ) {
      lang = fromExploreCookie;
    } else if ( fromPrefs ) {
      lang = fromPrefs;
    } else if (fromSession) {
      lang = fromSession;
