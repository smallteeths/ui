...
export default Service.extend({
  ...
  access:         service(),
  loadHarborServerUrl() {
    return get(this, 'globalStore').request({ url: '/v3/settings/harbor-server' }).then((resp) => {
  ...
  fetchHarborUserInfo() {
    return get(this, 'globalStore').request({ url: '/v3/settings/harbor-admin-auth' });
  },
  ...
  addWhitelist(ip) {
    ...
    return get(this, 'globalStore').request({ url }).then((resp) => {
      const wl = resp.value.split(',');
  ...
  saveHarborAccount(url, u, p) {
    const updateServerUrl = get(this, 'globalStore').request({
      url:    '/v3/settings/harbor-server',
      method: 'put',
      data:   JSON.stringify({ value: url }),
    });
    const updateAuth = get(this, 'globalStore').request({
