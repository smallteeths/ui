...
export default Component.extend({
  ...
  init() {
    ...
    } else if (cloudProviderName === 'external') {
      setProperties(this, { selectedCloudProvider: 'external' });
  ...
  selectedCloudProviderDisplayName: computed('selectedCloudProvider', 'intl.locale', function(){
    const providers = {
      none:      'generic.none',
      amazonec2: 'cloudProvider.amazon',
      azure:     'cloudProvider.azure',
      generic:   'generic.custom',
      external:  'cloudProvider.external.label',
    };

    const p = get(this, 'selectedCloudProvider');
    const label = providers[p];

    if (label) {
      return get(this, 'intl').t(label)
    }
