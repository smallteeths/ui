...
export default Component.extend(ThrottledResize, {
  ...
  willDestroyElement() {
    ...
    const chartEle = get(this, 'chartEle');
    ...
    if (chartEle && chartEle.onmousemove){
      chartEle.onmousemove = null
  ...
  create() {
    if (!$(this.element).find('.content') || !$(this.element).find('.content').length){
      return;
    }
    const element = $(this.element).find('.content')[0];
