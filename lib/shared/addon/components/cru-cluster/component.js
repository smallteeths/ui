...
const OWNER_NAME = 'objectset.rio.cattle.io/owner-name';
...
export default Component.extend(ViewNewEdit, ChildHook, {
  ...
  actions: {
    ...
    close(saved) {
      if (this.close) {
        ...
        if ( isEmbedded ) {
          window.top.postMessage({ action: 'need-redirect-to-cluster-index' });
  ...
  disabledName: computed('cluster.{annotations,annotations[OWNER_NAME],name}', 'isEdit', function(){
    return get(this, 'cluster.annotations') && this.cluster.annotations[OWNER_NAME] && this.isEdit
  }),
  ...
  operatorsettings: computed('model.operatorsettings.@each.{id,state}', function(){
  ...
  providerChoices: computed(
    'app.proxyEndpoint', 'cluster.rancherKubernetesEngineConfig', 'operatorsettings', 'intl.locale', 'isEdit', 'kontainerDrivers.[]', 'model.nodeDrivers', 'nodeDrivers.[]', 'schemaReloaded',
    function() {
      const { kontainerDrivers, intl  } = this;
      const { builtIn, custom } = kontainerDrivers;
      const operatorsettings = this.operatorsettings;
      ...
      if (operatorsettings.length > 0){
        operatorsettings.forEach((o) => {
          const driverName = `${ o.id.substring(0, o.id.lastIndexOf('operatorsetting')) }`;

          out.pushObject({
            displayName: driverName.toUpperCase(),
            driver:      get(o, 'id'),
            name:        get(o, 'name'),
            genericIcon: true
          });
      ...
      out.push({
        name:     'importack',
        driver:   'ackoperatorsetting',
        preSave:  false,
      });
      out.push({
        name:     'importtke',
        driver:   'tkeoperatorsetting',
        preSave:  false,
      });
      out.push({
        name:     'importcce',
        driver:   'cceoperatorsetting',
        preSave:  false,
      });
  ...
  driverInfo: computed('provider', 'providerChoices', 'router.currentRoute.queryParams', function() {
    ...
    if (name === 'import') {
      if (isEmpty(importProvider)) {
        ...
        name = 'importaks'
      } else if (importProvider === 'ack') {
        name = 'importack'
      } else if (importProvider === 'tke') {
        name = 'importtke'
      } else if (importProvider === 'cce') {
        name = 'importcce'
      }
    }
    ...
    if (name === 'tkeoperatorsetting'){
      name = 'tkeoperatorsetting'
    }
  ...
  validate() {
    ...
    const { intl } = this
    const { fluentdLogDir } = model

    if (!fluentdLogDir) {
      errors.pushObject(`${ intl.t('loggingPage.additional.fluentdLogDir.label') }${ 'generic.isRequired' }`)
    }
