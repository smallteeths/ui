...
export default Component.extend(ModalBase, NewOrEdit, {
  ...
  init() {
    ...
    const clone = get(this, 'originalModel');
  ...
  validate(){
    ...
    if (!originalModel.name){
      ...
      if (this.isNew){
        if (createdProviders.indexOf(get(originalModel, 'name')) > -1){
          errors.push(this.intl.t('operatorPage.provider.existError', { provider: this.providerChoices.find((item) => item.value === get(originalModel, 'name'))?.label || '' }));
        }
      }
    }
