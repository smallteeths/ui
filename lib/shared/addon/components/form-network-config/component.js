...
const NETWORKCHOICES = [
  {
    label: 'clusterNew.rke.network.none',
    value: 'none'
  },
  {
...
export default Component.extend({
  ...
  windowsSupportChange: observer('cluster.windowsPreferedCluster', function() {
    ...
    if ( !config || !get(config, 'network.options')) {
      return;
    }
  ...
  flannelBackendDidChange: observer('cluster.windowsPreferedCluster', 'config.network.plugin', 'config.network.options.flannel_backend_type', function() {
    ...
    if (config) {
      const network        = get(config, 'network');
      const plugin         = get(config, 'network.plugin');

      if (plugin !== NONE && !network.options){
        this.resetNetworkOptions();
      }
  ...
  projectNetworkIsolationAvailable: computed('config.network.plugin', function() {
    const plugin = get(this, 'config.network.plugin');
    const networkPolicySafe = [CANAL, CALICO, WEAVE, NONE];

    return networkPolicySafe.includes(plugin);
  }),

  configNetworkPluginDisplayName: computed('config.network.{options,plugin}', 'intl.locale', 'networkContent', function() {
    let plugin = get(this, 'config.network.plugin');
    ...
    const pluginOpt = get(this, 'networkContent').findBy('value', plugin)
  ...
  showMTU: computed('config.network.plugin', 'networkContent', function() {
    const hideValueArr            = ['none', 'flannel'];
    const plugin                  = get(this, 'config.network.plugin');
  ...
  resetNetworkOptions(){
    ...
    set(network, 'options', { flannel_backend_type: DEFAULT_BACKEND_TYPE });
    set(network, 'mtu', 0)
    set(this, 'cluster.windowsPreferedCluster', false)
  },
});
