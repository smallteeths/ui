...
export default Component.extend({
  ...
  setVethMtu: observer('cluster.rancherKubernetesEngineConfig.network.mtu', 'config.network.plugin', function() {
    ...
    if (plugin === 'multus-canal-macvlan') {
      if (!get(this, 'config.network.options')){
        this.resetNetworkOptions();
      }
  ...
  networkPluginArr: computed('networkContent', 'unsupportedMacvlan', function() {
  ...
  configNetworkPluginDisplayName: computed('config.network.{options,plugin}', 'intl.locale', 'networkPluginArr', function() {
    let plugin = get(this, 'config.network.plugin');
  ...
  showMTU: computed('config.network.plugin', 'networkPluginArr', function() {
    const hideValueArr            = ['none', 'flannel', 'multus-flannel-macvlan'];
    const plugin                  = get(this, 'config.network.plugin');
