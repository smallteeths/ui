...
export default Component.extend({
  ...
  quotaPercentData: computed('intl.locale', 'namespaceQuotas.[]', 'quotaKey', 'quotaTotal', 'usedProp', function() {
    ...
    let usedProgressValue = '';
    let totalProgressValue = '';
    let quotaTotal = convertToLimit(get(this, 'quotaKey'), get(this, 'quotaTotal'));
    ...
    if (!used){
      ...
      if (quotaTotal === total){
        usedPercent = Math.max(0.01, (used / total).toFixed(2));
        usedPercent = Math.min(0.99, usedPercent);
        usedProgressValue = usedPercent * total;
      } else {
        usedPercent = Math.max(0.01, (used / quotaTotal).toFixed(2));
        usedPercent = Math.min(0.99, usedPercent);
        usedProgressValue = usedPercent * quotaTotal;
      }
    }
    totalProgressValue = total - usedProgressValue;

    const value =  [
      {
        color: 'bg-primary',
        label,
        value: usedProgressValue,
      },
      {
        color: 'bg-info',
        label,
