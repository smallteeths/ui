...
export default SearchableSelect.extend({
  ...
  search: task(function * (term) {
    ...
    if ( get(this, 'includeLocal') ) {
      ...
      const principalPromises = local.filter((u) => !globalStore.getById('principal', u.principalIds[0]))
        .map((u) => globalStore.find('principal', u.principalIds[0]));

      if (principalPromises.length > 0) {
        try {
          set(this, 'loading', true);
          yield all(principalPromises);
        } catch (error) {
          console.error(error);
        } finally {
