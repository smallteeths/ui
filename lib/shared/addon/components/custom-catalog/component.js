...
export default Component.extend({
  ...
  finogeeks: computed('catalogs.@each.name', function() {
    return get(this, 'catalogs').findBy('name', C.CATALOG.FINOGEEKS_KEY)
  }),

  rows: computed('alibabaAppHub', 'catalogs.@each.{id,name,url}', 'finogeeks', 'helm3Stable', 'helmIncubator', 'helmStable', 'library', 'mode', function() {
    ...
    if ( get(this, 'mode') === 'global' ) {
      ...
      if ( !this.helmStable ) {
        out.push(get(this, 'globalStore').createRecord({
          ...
          helmVersion: C.CATALOG.HELM_VERSION_3,
        }));
      }
      ...
      //   out.push(get(this, 'globalStore').createRecord({
      //     type:   'catalog',
      //     name:   C.CATALOG.FINOGEEKS_KEY,
      //     url:    C.CATALOG.FINOGEEKS_VALUE,
      //     branch: C.CATALOG.DEFAULT_BRANCH,
      //     kind:   'helm',
      //   }));
      // }
