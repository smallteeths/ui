...
export default Component.extend(ThrottledResize, {
  ...
  exec() {
    ...
    if ( !url ) {
      ...
      endpoint = `/k8s/clusters/${ clusterId }`
    }
    if (get(this, 'instance.modalService.modalOpts.permissionCheck')) {
      this.hasPermission(endpoint, namespaceId, podName)
        .then((resp = {}) => {
          if (get(resp, 'body.status.allowed')) {
            this.connect(url);
          } else {
            set(this, 'status', statusMap.closed);
            set(this, 'errors', [this.intl.t('containerShell.permissionDenied')]);
          }
        })
        .catch((err) => {
          set(this, 'status', statusMap.closed);
          set(this, 'errors', [JSON.stringify(err)]);
        });
    } else {
      this.connect(url);
    }
  ...
  disconnect(destroying = false) {
    ...
    if (socket) {
      try {
        socket.send(`0${ AWS.util.base64.encode('\x04') }`); // send ctrl+d(EOT) https://en.wikipedia.org/wiki/Control_character
      } catch (err) {
        // console.error(err);
      }
      socket.close();
  ...
  hasPermission(endpoint, namespace, name) {
