...
export default InputTextFile.extend(ClusterDriver, {
  ...
  isArm64:              equal('model.cluster.arch', 'arm64'),
  ...
  archChanged: observer('model.cluster.arch', function() {
    const config = get(this, 'cluster.rancherKubernetesEngineConfig')

    if (get(this, 'isArm64')) {
      const res = NETWORKCHOICES.filter((n) => n.value === FLANNEL)
    ...
    } else if (!get(this, 'windowsSupport')){
      set(this, 'windowsSupport', false)
