...
const CUSTOM_OBJ_TO_YAML_KEY_MAP = {
  enableGPUManagement: 'enable_gpu_management',
  enableF5CIS:         'enable_f5_cis'
}

const CUSTOM_YAML_TO_OBJ_KEY_MAP = {
  enable_gpu_management: 'enableGPUManagement',
  enable_f5_cis:         'enableF5CIS'
}
...
export default InputTextFile.extend(ManageLabels, ClusterDriver, {
  ...
  enableGPUManagementDidChanged: observer('cluster.enableGPUManagement', function() {
    // remove gpu sharing config for compatible with older versions
  ...
  getClusterFields(primaryResource) {
    ...
    decamelizedObj = keysToDecamelize(pojoPr, void (0), ['type', 'azureCloudProvider'], CUSTOM_OBJ_TO_YAML_KEY_MAP);
  ...
  parseOptsFromYaml(yamlValue) {
    ...
    return keysToCamel(yamlConfig, '', CUSTOM_YAML_TO_OBJ_KEY_MAP);
  },
  ...
  getSupportedFields(source, targetField, excludeFields = []) {
    ...
    const customMap = CUSTOM_OBJ_TO_YAML_KEY_MAP
    ...
    Object.keys(resourceFields).filter((key) => resourceFields[key].update && excludeFields.indexOf(key) === -1).forEach((key) => {
      ...
      out[customMap && customMap[key] ? customMap[key] : camelToUnderline(key)] = value;
    });
  ...
  updateClusterAnnotations() {
    // update annotation gpumanagement.pandaria.io/appAnswers
    const key = 'gpumanagement.pandaria.io/appAnswers'
    const annotations = get(this, 'cluster.annotations') || {};
    let appAnswersStr = annotations[key];
    let appAnswers = {};

    if (appAnswersStr) {
      try {
        appAnswers = JSON.parse(appAnswersStr)
      } catch (error) {
        console.warn(`Parse cluster annotations['${ key }'] error: `, error);
      }
    }
    let answers = appAnswers.answers;

    if (get(this, 'cluster.enableGPUManagement')) {
      if (!answers) {
        answers = {};
        appAnswers.answers = answers;
      }
      answers['image.defaultScheduler.version'] = (get(this, 'config.kubernetesVersion') || get(this, 'initialVersion')).split('.').slice(0, 2).join('.');
      annotations[key] = JSON.stringify(appAnswers);
    } else {
      if (answers) {
        delete answers['image.defaultScheduler.version'];
        if (Object.keys(answers).length === 0) {
          delete appAnswers.answers;
        }
      }

      if (Object.keys(appAnswers).length === 0) {
