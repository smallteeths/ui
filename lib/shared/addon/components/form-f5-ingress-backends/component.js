...
export default Component.extend({
  ...
  servicesDidChange: observer('temp.service', function() {
    ...
    if (!get(temp, 'service') || get(temp, 'backendType') !== 'service') {
      return;
    }

    const servicePort = get(temp, 'servicePort');
    const numberTest = /^[0-9]+.?[0-9]*$/;
    const availablePorts = (get(temp, 'service.availablePorts') || []).filter((p) => numberTest.test(p.port));
    const hasPorts = availablePorts.length > 0;

    set(temp, 'service.availablePorts', availablePorts);

    if (hasPorts && !availablePorts.find((p) => p.port === servicePort)) {
  ...
  setPool() {
    ...
    if (path) {
      setProperties(get(this, 'pool'), { path })
    } else {
