...
export default Component.extend({
  ...
  actions: {
    add() {
      ...
      next(() => {
        ...
        const elem = $(this.element).find('INPUT.value').last()[0];
    ...
    pastedValues(index, str) {
      ...
      set(ary[index], 'value', `${ ary[index].value }${ lines[0] }`)
  ...
  initialValuesDidChanged: observer('initialValues', 'editing', function() {
    if (this.editing) {
      return
    }
    var ary = [];
