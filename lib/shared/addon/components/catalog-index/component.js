...
export default Component.extend({
  ...
  groupedContent: computed('arrangedContent.[]', 'catalogs.@each.name', function() {
    ...
    all.forEach((template) => {
      const needHidden = template.labels && template.labels[C.LABEL.HIDDEN] === 'true'
    ...
    function getOrCreateGroup(name){
      ...
      if ( name === C.CATALOG.LIBRARY_KEY ) {
        priority = 1;
      } else if ( [C.CATALOG.HELM_STABLE_KEY, C.CATALOG.HELM_INCUBATOR_KEY, C.CATALOG.ALIBABA_APP_HUB_KEY, C.CATALOG.FINOGEEKS_KEY].includes(name) ) {
        priority = 2;
      }
