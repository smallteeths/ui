...
export default Component.extend(ModalBase, {
      ...
      eachLimit(resources, 5, (resource, cb) => {
        ...
        if ( resource.cb ) {
          ...
          if ( get(this, 'forceDelete') ) {
            if (get(this, 'isNamespace')){
              get(this, 'globalStore').rawRequest({
                url:     get(resource, 'actionLinks.clearAllFinalizers'),
                method:  'POST',
                data:   {},
              }).then(() => {
                if (resource.state !== 'removing'){
  ...
  nsSupportForceDelete: computed('isNamespace', 'resource', 'resources', function() {
    return get(this, 'resources').every((item) => {
      return get(item, 'type') === 'namespace' && get(item, 'actionLinks.clearAllFinalizers')
    });
  }),
