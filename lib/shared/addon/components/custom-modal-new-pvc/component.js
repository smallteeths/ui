...
export default Component.extend(ModalBase, {
  ...
  init() {
    ...
    if (get(this, 'model.storageClassId')) {
      const sc = this.storageClasses.find((item) => item.id === get(this, 'model.storageClassId'));

      if (!sc) {
        set(this, 'model.storageClassId', null);
      }
    }
  },
  ...
  validate() {
    ...
    if ( get(this, 'useStorageClass') ) {
      ...
      const pv = get(this, 'persistentVolumes').findBy('id', pr.volumeId) || {};

      set(pr, 'resources', { requests: Object.assign({}, get(pv, 'capacity')) });
    }
