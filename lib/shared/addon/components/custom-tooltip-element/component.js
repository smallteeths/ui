...
export default Component.extend({
  ...
  show(node) {
    ...
    let out = {
      type:          this.get('type'),
      baseClass:     this.isOverView(node) ? `${ this.get('baseClass')  } last` : this.get('baseClass'),
  ...
  isOverView(ev){
    const event = ev[0] || {};
    const visibleBottom = window.scrollY + document.documentElement.clientHeight - 130;
    // const visibleTop = window.scrollY;
    const centerY = ev.offset().top + (event.offsetHeight);

    return centerY > visibleBottom
  }
});
