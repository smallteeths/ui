...
export default Component.extend(Upload, {
  ...
  init() {
    ...
    // disabled key and value when secret name is alertmanager-cluster-alerting

    if (get(this, 'secretName') === 'alertmanager-cluster-alerting' || get(this, 'secretName') === 'alertmanager-default-notification-template') {
      ary.forEach((entry) => {
        if (entry.key === 'notification.tmpl' || entry.key === 'alertmanager.yaml') {
  ...
  actions: {
    add() {
      ...
      next(() => {
        ...
        let elem = $(this.element).find('INPUT.key').last()[0];
  ...
  initToggleChange: on('init', observer('initToggle', function() {
    this.init()
  })),
