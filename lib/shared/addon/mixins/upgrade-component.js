...
function versionGt(a = '', b = '') {
  ...
  const count = Math.min(aVersions.length, bVersions.length);

  for (let i = 0; i < count; i++) {
    const aPart = parseInt(aVersions[i], 10);
    const bPart = parseInt(bVersions[i], 10);
  ...
  if (aVersions[count - 1].length > bVersions[count - 1].length){
    return true;
  }
  ...
  updateStatus() {
    ...
    if ( catalogTemplate ) {
      ...
      if (Object.keys(upgradeVersions).length >= 1) {
        ...
        const versions = Object.keys(upgradeVersions)
      ...
      keys.forEach( (k) => {
        try {
          ...
          if ( v !== null && cv !== null ) {
            ...
            let gt = versionGt(k, currentVersion);
