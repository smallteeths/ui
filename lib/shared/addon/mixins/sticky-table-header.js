...
export default Mixin.create(ThrottledResize, {
  ...
  positionHeaders() {
    ...
    $fixedHeader.css({
      'position': 'fixed',
      'top':      `${ (showHeader && this.get('bulkActions')) || this.get('hasSelect') ? fudge + tableProps.actionsHeight : 0  }px`,
      'height':   `${ tableProps.fixedHeaderHeight  }px`,
    });

    elem.css({ 'padding-top': `${ ( (showHeader && this.get('bulkActions')) || this.get('hasSelect') ? tableProps.actionsHeight : 0) + tableProps.fixedHeaderHeight  }px` });
  },
