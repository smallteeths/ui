...
export default Mixin.create({
  ...
  init() {
    ...
    socket.on('message', (event) => {
      schedule('actions', this, function() {
        ...
        if (d.name === 'resource.change' && d.data.type === 'etcdBackup' && (d.data.state === 'failed' || d.data.state === 'active')){
          this.etcdBackupPrompt(d.data);
        }
      });
    });
  ...
  etcdBackupPrompt(data){
    if (get(data, 'annotations') && get(data, 'status.conditions.lastObject.type') === 'Completed'){
      const time = new Date().getTime();
      const CompletedTime = new Date(get(data, 'status.conditions.lastObject.lastUpdateTime')).getTime();
    ...
    const getBackupType = () => {
      if (isEmpty(get(data, 'backupConfig.s3BackupConfig'))) {
        return 'local';
      } else if (!isEmpty(get(data, 'backupConfig.s3BackupConfig'))) {
        return 's3';
      }
    }

    const currentCluster = this.scope.allClusters.filterBy('id', data.clusterId)[0];
    const displayName    = currentCluster && currentCluster.displayName || data.clusterId;
    const backupType     = getBackupType();
    const state          = data.state === 'failed' ? 'fail' : 'success';
    const title          = `${ this.intl.t(`action.backupEtcdMessage.${ state }.title`) }: ${ data.name }`;
    const message        = this.intl.t(`action.backupEtcdMessage.${ state }.message`, {
      clusterId:  displayName,
      backupType
    });

    data.state === 'failed' ? this.growl.fromError(title, message) : this.growl.success(title, message);
  }
});
