...
export default Mixin.create({
  ...
  update(cb) {
    ...
    const currentApp = get(this, 'apps.firstObject') || app;
    const currentVersion   =  get(currentApp, 'externalIdInfo.version');
  ...
  initWorkloads() {
    this.getEnalbedWorkloads().map((w) => {
      defineProperty(this, `${ w }SchedulableNodes`, computed(`${ w }NodeSelectors.[]`, 'cluster.nodes.@each.{allocatable,requested}', function() {
      ...
      defineProperty(this, `${ w }Warning`, computed(`insufficient${ ucFirst(w) }Cpu`, `insufficient${ ucFirst(w) }Memory`, function() {
        return this.getComponentWarning(w)
      }))
    });
  },

  totalWarning: computed('insufficientCpu', 'insufficientMemory', 'intl.locale', 'warningPrefix', function() {
    let {
