...
export default Service.extend({
  ...
  settings:       service(),
  intl:           service(),
  auditLogServer: computed('settings.asMap', function() {
  ...
  fetchWorkloadAuditLogs(clusterID, projectID, params) {
    const query = Object.entries(params).map((e) => `${ e[0] }=${ e[1] }`).join('&');
    const logs = this.clusterStore.findAll('namespace').then((namespaces) => {
      const nsIds = namespaces.filter( (ns) => get(ns, 'projectId') === get(this, 'scope.currentProject.id'))
        .map((ns) => ns.id);

      return get(this, 'globalStore').rawRequest({
        url:    `/meta/auditlog/${ get(this, 'auditLogServer').replace('//', '/') }/v1/projectauditlogs?clusterID=${ clusterID }&projectID=${ projectID }&${ query }`,
        method: 'POST',
        data:    { namespaces: nsIds },
      });
    });
