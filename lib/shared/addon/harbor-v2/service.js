...
export default Service.extend({
  ...
  getHarborConfigurations(){
    const url = `/meta/harbor/${ get(this, 'harbor.harborServer').replace('//', '/').replace(/\/+$/, '') }/api${ get(this, 'harborVersion') }/configurations`;
    ...
    return get(this, 'globalStore').rawRequest({
      ...
      const saveHarborConfig = get(this, 'globalStore').rawRequest({
        ...
        data:   JSON.stringify({
          ...
          username:  u,
          password:  get(this, 'access').encryptPassword(p),
          version:   v,
        }),
      }).then(() => {
        const updateServerUrl = get(this, 'globalStore').rawRequest({
  ...
  syncHarborAccount(params) {
    ...
    if (data.password) {
      data.password = get(this, 'access').encryptPassword(data.password)
    }
  ...
  deleteRepos(names, project_name){
    const promises = names.map((n) => {
      return get(this, 'globalStore').rawRequest({
        url:     `/meta/harbor/${ get(this, 'harbor.harborServer').replace('//', '/').replace(/\/+$/, '') }/api${ get(this, 'harborVersion') }/projects/${ project_name }/repositories${ n.replace(project_name, '') }`,
        headers: { 'X-API-Harbor-Admin-Header': !!get(this, 'access.me.hasAdmin') },
        method:  'DELETE',
      });
    });
  ...
  fetchTags(p) {
    ...
    let repoName = p.repository_name.replace(`${ p.project_name }/`, '').replace('/', '%252F')

    return get(this, 'globalStore').rawRequest({
      headers: { 'X-API-Harbor-Admin-Header': !!get(this, 'access.me.hasAdmin') },
      url:     `/meta/harbor/${ get(this, 'harbor.harborServer').replace('//', '/').replace(/\/+$/, '') }/api${ get(this, 'harborVersion') }/projects/${ p.project_name }/repositories/${ repoName }/artifacts?with_tag=true&with_scan_overview=true&with_label=true&${ params }`,
    })
  },
  ...
  updateHarborPwd(userId, params) {
    const data = {
      ...params,
      newPassword: get(this, 'access').encryptPassword(params.newPassword),
      oldPassword: get(this, 'access').encryptPassword(params.oldPassword),
    }
  ...
  fetchConnectHarbor(baseUrl){
    const url = `/meta/harbor/${ baseUrl.replace('//', '/').replace(/\/+$/, '') }/api${ get(this, 'harborVersion') }/systeminfo`;
  ...
  saveHarborAccountWhenVersionOk(url, u, p, v, resp) { // for admin user
    const data = resp.body || {};
    const authMode = data.auth_mode;
    const saveHarborConfig = get(this, 'globalStore').rawRequest({
      url:    '/v3/users?action=saveharborconfig',
