...
const EXPOSED_OPTIONS = ['exporter-node.enabled', 'exporter-node.ports.metrics.port',
  ...
  'exporter-gpu-node.enabled', 'exporter-kube-state.enabled',
  'exporter-kubelets.enabled', 'exporter-kubernetes.enabled',
  'exporter-fluentd.enabled',
];
...
const QUESTIONS = [
  ...
  'exporter-kube-state.enabled',
  'exporter-kubelets.enabled',
  'exporter-kubernetes.enabled',
  'exporter-fluentd.enabled',
]
...
export default Component.extend(ReservationCheck, CatalogUpgrade, InputAnswers, {
    ...
    resource.waitForAction('viewMonitoring').then(() => {
      ...
      resource.doAction('viewMonitoring').then((res) => {
        ...
        Object.keys(body || {}).forEach((key) => {
          ...
          } else if (!Object.keys(CLUSTER_HIDDEN_KEYS).includes(key) && !key.startsWith('prometheus.thanos')) {
  ...
  updateConfig(answers) {
    ...
    if ( get(this, 'level') === 'cluster' ) {
      ...
      if ( extraAnswers['exporter-gpu-node.enabled'] !== undefined && answers['exporter-gpu-node.enabled'] === undefined) {
        set(this, 'enableGpuMonitoring', extraAnswers['exporter-gpu-node.enabled'] === 'true');
      } else if (typeof answers['exporter-gpu-node.enabled'] === 'string') {
        set(this, 'enableGpuMonitoring', answers['exporter-gpu-node.enabled'] === 'true');
      } else {
        set(this, 'enableGpuMonitoring', answers['exporter-gpu-node.enabled']);
      }
    }
  ...
  updateAnswers() {
    let answers = get(this, 'level') === 'cluster' ? {
      ...CLUSTER_HIDDEN_KEYS,
      ...DEFAULT_ANSWERS,
    } : { ...DEFAULT_ANSWERS };
    ...
    if ( get(this, 'level') === 'cluster' ) {
      ...
      answers['exporter-gpu-node.enabled'] = `${ get(this, 'enableGpuMonitoring') }`;
