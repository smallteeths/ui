  ...
  fetchPods() {
    ...
    this.globalStore.rawRequest({
      ...
      .then((xhr) => {
        ...
        const pods = (xhr.body.items || []).map((item) => {
          const pod = item
          ...
          const wrapper = this;
          const specContainers = get(pod, 'spec.containers') || []

          let restartCount = 0;

          let displayImages = [];

          if (get(totalContainers, 'length') > 0) {
            displayImages = totalContainers.map((containerItem) => {
              return containerItem.image;
            });
          } else {
            displayImages = specContainers.map((c) => {
              return c.image;
            });
          }
          ...
          } else if (get(specContainers, 'length') > 0) {
            if (get(specContainers, 'length') > 1) {
              displayImage = this.intl.t('podPage.displayImage', {
                image:   get(specContainers, 'firstObject.image'),
                sidecar: get(specContainers, 'length') - 1
              });
            } else {
              displayImage = get(specContainers, 'firstObject.image');
            }
          } else {
          ...
          return {
            ...
            _availableActions: [
              ...
              },
              {
                ...
                enabled:    !!containers.findBy('canShell', true),
            ...
            containerName:   get(containers, 'firstObject.name'),
            showActions:     true,
