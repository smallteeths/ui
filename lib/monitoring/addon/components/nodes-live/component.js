...
export default NodesReservation.extend(Metrics, {
  ...
  // pandaria
  cpuLiveTitleTooltip: computed('nodeCapcity', 'graphs.[]', 'cluster.capacity', 'intl.locale', function() {
    const { graphs, nodes = [] } = this
    ...
    total = `${ formatSi(total, 1000, '', '', 0, exponentNeeded(total), 1) * 1000 } milli CPU`

    return get(this, 'intl').t('clusterDashboard.liveTitle', {
      total,
      used: `${ formatSi(used, 1000, '', '', 0, exponentNeeded(used), 1) * 1000 } milli CPU`,
    })
  }),

  // pandaria
  meomoryLiveTitleTooltip: computed('nodes.@each.memory', 'memoryLive', 'intl.locale', function() {
    const { graphs, nodes = [] } = this
    ...
    return get(this, 'intl').t('clusterDashboard.liveTitle', {
      total: formatSi(total, 1024, 'iB', 'B', 0, exponentNeeded(total), 1),
      used:  formatSi(used, 1024, 'iB', 'b', 0, exponentNeeded(used), 1),
    })
  }),
