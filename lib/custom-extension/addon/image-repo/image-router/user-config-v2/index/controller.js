...
export default Controller.extend({
  ...
  actions:                 {
    ...
    confirmChangeHarborAccount(cb) {
      ...
      get(this, 'harborV2').updateHarborPwd(userId, form).then(() => {
        cb(true);
        // set(this, 'harborAccountState', syncedHarborAccount);
        location.reload();
      }).catch((err) => {
        cb(false);
        set(this, 'changeAccountErrors', [Errors.stringify(err.body)]);
      });
    },
    ...
    syncAccount(cb) {
      ...
      if (!get(this, 'isDBAuthMode')) {
        ...
        if (get(this, 'requiredAuth')) {
          ...
          form.email = email
          if (this.rancherAuthMode === 'KEYCLOAK_USER' && this.isDBAuthMode) {
            form.username = username
            form.password = password
          }
        }
      ...
      get(this, 'harborV2').syncHarborAccount(form).then(() => {
        ...
        set(this, 'errors', [Errors.stringify(err && err.body)]);
        cb(false);
      });
    }
  },
  testHarborAccount: observer('hasHarborRegistry', 'harborAccountState', function() {
    ...
    if (get(this, 'harborAccountState') === syncedHarborAccount && get(this, 'hasHarborRegistry')) {
      ...
      return get(this, 'harborV2').testHarborAccount(url).then((resp) => {
        ...
        set(this, 'harborAccountErrors', [JSON.stringify(err && err.body && err.body.errors[0])]);
      });
    }
  }),
  ...
  configActions: computed('harborAccountState', 'hasHarborAccountError', 'hasHarborRegistry', 'isDBAuthMode', 'methodNotSupported', function() {
    if (this.methodNotSupported) {
      return []
    }

    if (get(this, 'hasHarborRegistry') && get(this, 'harborAccountState') === syncedHarborAccount && !get(this, 'hasHarborAccountError') && get(this, 'isDBAuthMode')) {
      return [
        {
  ...
  showActionMenu: computed('configActions.length', function() {
    return get(this, 'configActions.length') !== 0;
  }),
  isDBAuthMode: computed('model.harborSystemInfo.auth_mode', function() {
  ...
  methodNotSupported: computed('access.principal.id', 'model.harborSystemInfo.auth_mode', function() {
