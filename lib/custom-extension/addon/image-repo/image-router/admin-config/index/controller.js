...
export default Controller.extend({
  ...
  actions:                  {
    ...
    saveHarborConfig(cb) {
      set(this, 'harborAccountErrors', null);
      if (get(this, 'harborConfig.url')) {
        set(this, 'harborConfig.url', get(this, 'harborConfig.url').trim())
      }
      ...
      const orginSettingValue = get(this, 'settings.harbor-insecure-skip-verify') ?? false;

      this.settings.set('harbor-insecure-skip-verify', this.harborInsecureSkipVerify);
      this.settings.one('settingsPromisesResolved', () => {
        this.addWhitelist().then(() => {
          ...
          return get(this, selected.service).fetchConnectHarbor(url).then((harborInfo) => {
            ...
            this.settings.set('harbor-insecure-skip-verify', orginSettingValue);
    ...
    removeHarboConfig() {
      ...
      this.settings.set('harbor-insecure-skip-verify', false);
      this.settings.one('settingsPromisesResolved', () => {
  ...
  methodNotSupported: computed('access.principal.id', 'model.harborSystemInfo.auth_mode', 'model.harborUser.invalid', function() {
