  ...
  actions:                 {
    ...
    saveHarborConfig(cb) {
      ...
      this.addWhitelist().then(() => {
        ...
        const harbor1 = get(this, 'harbor').fetchConnectHarbor(url).catch((err) => err);
        const harbor2 = get(this, 'harborV2').fetchConnectHarbor(url).catch((err) => err);

        return Promise.all([harbor1, harbor2]).then((data) => {
          const [harbor1, harbor2] = data;

          if (harbor1.status !== 200 && harbor2.status !== 200){
            return reject(version === 'v2.0' ? harbor2 : harbor1);
          }

          if ((version === 'v2.0' && harbor2.status === 200) || (version === '' && harbor1.status === 200)){
            const service = version === 'v2.0' ? 'harborV2' : 'harbor';
            const harborInfo = version === 'v2.0' ? harbor2 : harbor1;

            return get(this, service).saveHarborAccountWhenVersionOk(url, username, password, version, harborInfo).then((resp) => {
