...
export default Controller.extend({
  ...
  availableActions: [
    {
      ...
      active(selectedRows) {
        ...
        return get(this, 'model.currentUser.sysadmin_flag') || selectedRows.every((pro) => `${ pro.current_user_role_id }` === '1');
      },
    },
  ],
  init() {
  ...
  actions:     {
    ...
    confirmDelete() {
      ...
      if (projects && projects.length > 0) {
        get(this, 'harborV2').removeProjects(projects.map((p) => p.project_id)).then(() => {
          ...
          if (err.body && err.body.errors && err.body.errors[0] && err.body.errors[0].message) {
            this.growl.fromError('删除失败', err.body.errors[0].message)
          }
        });
      }
    },
  ...
  selectChange: observer('isPublic', function() {
    set(this, 'page', 1);
  }),
  ...
  isAdmin: computed('model.currentUser.sysadmin_flag', function() {
  ...
  data: computed( 'model.projects.data', function() {
    ...
    return rawData.map((item) => {
      if (item.current_user_role_ids === null){
        item.current_user_role_id = 0
      }
