...
export default Route.extend({
  ...
  model(params) {
    ...
    if (get(this, 'harborServer')) {
      ...
      const configurations = get(this, 'harborV2').getHarborConfigurations().then((resp) => {
        if (resp.body && resp.body.storage_per_project && resp.body.storage_per_project.value && resp.body.storage_per_project.value !== -1) {
          return this.unitConversions(resp.body.storage_per_project.value)
        }

        return {
          defaultProjectQuota: -1,
          unit:                'GB'
        }
      }).catch(() => {
        return {
      ...
      return hash({
        ...
        // Quota cannot be set for non-admin users
        if (!hash.currentUser.sysadmin_flag) {
          set(hash, 'configurations', {
            defaultProjectQuota: -1,
            unit:                'GB'
          })
        }
  ...
  unitConversions(size) {
    ...
    if (sizeGb < 1) {
      return {
        defaultProjectQuota: sizeGb * 1024,
        unit:                'MB'
      }
    } else if ( 1 <= sizeGb && sizeGb < 1024 ) {
      return {
        defaultProjectQuota: sizeGb,
        unit:                'GB'
      }
    } else if (sizeGb >= 1024) {
      return {
