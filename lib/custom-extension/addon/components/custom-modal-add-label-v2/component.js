...
export default Component.extend({
  ...
  actions:               {
    ...
    save(cb) {
      ...
      label.project_id = parseInt(label.project_id, 10);
      get(this, 'harborV2').createLabel(label).then(() => {
        this.send('cancel');
        cb(true);
        ...
        if (err.status === 409) {
          errors.push(intl.t('harborConfig.formTag.name.nameExist'));
        } else {
  ...
  validate() {
    const errors = [];
    const label = get(this, 'label.name');
    const intl = get(this, 'intl');

    if (!label) {
      errors.push(intl.t('harborConfig.formTag.name.nameReq'));
    }
    if (errors.length > 0) {
      set(this, 'errors', errors);
