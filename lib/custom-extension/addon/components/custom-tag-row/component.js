...
export default Component.extend({
  ...
  resourceActions: computed('currentUser.has_admin_role', 'currentUserRoleId', function() {
  ...
  pullCommand:  computed('harborServer', 'model.name', 'repo', function() {
  ...
  labels: computed('labelColors', 'model.labels', function() {
    ...
    return labels.map((label) => {
      ...
      return {
        cssStyle:   htmlSafe(`max-width:70px;font-size:12px;display:inline-block;padding:0 6px;border-radius: 6px;border:${ border };background-color:${ color }; color:${ font && font.textColor }`),
        name:       label.name,
