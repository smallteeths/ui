...
export default Route.extend({
  ...
  afterModel(model) {
    const enableLoadResourceByNamespace = get(this, 'settings.enable-load-resource-by-namespace')

    if (enableLoadResourceByNamespace) {
      const namespaceId = model.workloadForNew.workload.namespaceId;

      return Promise.all([
        this.nsResource.findAll('namespacedSecret', namespaceId),
        this.nsResource.findAll('secret'),
        this.nsResource.findAll('configMap', namespaceId),
      ]);
    }
  },
