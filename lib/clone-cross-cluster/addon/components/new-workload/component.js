...
export default Component.extend({
  ...
  ignoreNameConflicts:          false,
  confirmIgnoreNameConflicts:   false,
  ...
  init() {
    ...
    set(this, 'ignoreNameConflicts', get(this, `cookies.${ C.COOKIE.IGNORE_CRED_CLONE_WORKLOAD }`) === 'true');
  },
  ...
  actions: {
    ...
    updateSecret(secret, originSecret) {
      ...
      const types = ['secret', 'namespacedSecret'];

      [
        ...Object.values(this.envMap),
        ...Object.values(this.envFromMap)
      ].reduce((t, c) => {
        t.push(...c);

        return t;
      }, [])
        .filter((item) => item.refData && types.includes(item.refData.type) && item.refData._originName === secret._originName)
        .forEach((item) => {
          set(item, 'refData', secret);
          if (item.data.secretRef) {
            set(item, 'data.secretRef.name', secret.name);
          } else if (item.data.valueFrom && item.data.valueFrom.secretKeyRef){
            set(item, 'data.valueFrom.secretKeyRef.name', secret.name);
          }
        });
    ...
    updateConfigMap(configMap, originConfigMap) {
      ...
      [
        ...
        .filter((item) => item.refData && item.refData.type === 'configMap' && item.refData._originName === configMap._originName)
        .forEach((item) => {
          set(item, 'refData', configMap);
          if (item.data.configMapRef) {
            set(item, 'data.configMapRef.name', configMap.name);
          } else if (item.data.valueFrom && item.data.valueFrom.configMapKeyRef) {
            set(item, 'data.valueFrom.configMapKeyRef.name', configMap.name);
          }
        });
    ...
    async save(cb) {
      ...
      this.setContainersEnv(service.containers);
      ...
      if (workload.labels) {
        delete workload.labels['workload.user.cattle.io/workloadselector'];
      }
      ...
      if (this.ignoreNameConflicts && credentialList.length > 0) {
        try {
          const ingoreData = await this.checkCreds(credentialList);

          if (ingoreData.length > 0) {
            data.ignoreList = ingoreData.reduce((t, c) => {
              const types = t[c.type] || [];
        ...
        delete data.ignoreList;
      }

      try {
        await this.cloneApp.cloneApp(this.project.id, data);
  ...
  ignoreNameConflictsDidChanged: observer('ignoreNameConflicts', function() {
    this.cookies.set(C.COOKIE.IGNORE_CRED_CLONE_WORKLOAD, this.ignoreNameConflicts);
  }),
  ...
  allSecrets: computed('projectSecrets.[]', 'namespaceSecrets.[]', 'certificates.[]', 'namespacedCertificates.[]', 'service.namespaceId', function() {
    return [
      ...this.projectSecrets.slice(),
      ...this.namespaceSecrets.filter((item) => item.namespaceId === this.service.namespaceId).slice(),
      ...this.certificates.slice(),
      ...this.namespacedCertificates.filter((item) => item.namespaceId === this.service.namespaceId).slice()
    ];
  }),
  ...
  validatePermission() {
    const envVars = [
      ...
      return t;
    }, []).filter((item) => ((item.data.valueFrom && (item.data.valueFrom.configMapKeyRef || item.data.valueFrom.secretKeyRef)) || (item.data.configMapRef || item.data.secretRef)) && !item.refData);
    ...
    envVars.forEach((item) => {
      errors.push(intl.t('cloneCrossCluster.newContainer.errors.noPermissionAccessResource', {
        type: ((item.data.valueFrom && item.data.valueFrom.secretKeyRef) || item.data.secretRef) ? intl.t('cruPersistentVolume.secret.secretName.label') : intl.t('cruPersistentVolume.configMap.name.label'),
        name: item.data.secretRef ? item.data.secretRef.name : item.data.valueFrom.secretKeyRef.name,
      }));
    });
  ...
  initService() {
    ...
    this.initContainersEnv(containers);
  ...
  getSecretType(secretName) {
    ...
    found = this.certificates.findBy('name', secretName)

    if (found) {
      return C.VOLUME_TYPES.CERTIFICATE;
    } else {
      found = this.namespacedCertificates.findBy('name', secretName);
      if (found) {
        return C.VOLUME_TYPES.CERTIFICATE;
      }
    }
  ...
  initImagePullSecrets(imagePullSecrets = []) {
    const data = imagePullSecrets.reduce((t, c) => {
      if (!t.find((item) => item.name === c.name)) {
        t.push(c);
      }
  ...
  initContainersEnv(containers) {
    containers.forEach((c) => {
      set(this, `targetEnvMap.${ c._originName }`, []);
      set(this, `targetEnvFromMap.${ c._originName }`, []);

      const env = JSON.parse(JSON.stringify(c.env || []));
      const envFrom = JSON.parse(JSON.stringify(c.envFrom || []));

      set(this, `envMap.${ c._originName }`, env.map((item) => {
        if (item.valueFrom && item.valueFrom.configMapKeyRef) {
          return {
            data:    item,
            refData: this.findAndCloneConfigMap(item.valueFrom.configMapKeyRef.name)
          }
        } else if (item.valueFrom && item.valueFrom.secretKeyRef) {
          return {
            data:    item,
            refData: this.findAndCloneSecret(item.valueFrom.secretKeyRef.name)
          }
        } else {
          return { data: item };
        }
      ...
      set(this, `envFromMap.${ c._originName }`, envFrom.map((item) => {
        if (item.secretRef) {
          return {
            data:    item,
            refData: this.findAndCloneSecret(item.secretRef.name)
          }
        } else if (item.configMapRef) {
          return {
            data:    item,
            refData: this.findAndCloneConfigMap(item.configMapRef.name)
          }
        }
      }));
    });
  ...
  initWorkloadServices(allServices) {
    ...
    const customServiceList = allServices.filter((s) => {
      if (s.targetWorkloadIds && s.targetWorkloadIds.length > 0) {
  ...
  setContainersEnv(containers) {
    containers.forEach((c) => {
      set(c, 'env', [
        ...this.envMap[c._originName].map((item) => item.data),
        ...this.targetEnvMap[c._originName],
      ]);
      set(c, 'envFrom', [
        ...this.envFromMap[c._originName].map((item) => item.data),
        ...this.targetEnvFromMap[c._originName]
      ]);
    });
  },
  ...
  getSecretList() {
    ...
    const envSecrets = [
      ...
      return t;
    }, []).filter((item) => item.refData && types.includes(item.refData.type)).map((item) => item.refData);
    ...
    return out.map((item) => {
      ...
      if (result.namespaceId || result.type === 'namespacedSecret') {
        delete result.kind;
      }
  ...
  getConfigMapList() {
    const envConfigMaps = [
      ...
      return t;
    }, []).filter((item) => item.refData && 'configMap' === item.refData.type).map((item) => item.refData);
  ...
  checkCreds(credList) {
    return new Promise(async(resolve, reject) => { // eslint-disable-line
      try {
        const [{ body: creds }, { body: nsCreds }] = await Promise.all([this.cloneApp.loadCredentials(this.targetForm.project), this.cloneApp.loadNamespaceCredentials(this.targetForm.project, this.targetForm.namespace)]);
        const targetData = [...creds.data, ...nsCreds.data].filter((item) => credList.find((c) => c.name === item.name));

        if (targetData.length > 0 && !this.confirmIgnoreNameConflicts) {
          const sourceData = credList.filter((item) => targetData.find((c) => c.name === item.name));
          const modelData = targetData.map((item) => {
            const source = sourceData.find((c) => c.name === item.name);

            return {
              source: {
                ...source,
                registries: Object.entries(item.registries).map(([k, v]) => ({
                  address:  k,
                  username: v.username
                }))
              },
          ...
          get(this, 'modalService').toggleModal('modal-confirm-ignore-name-conflicts', {
            model:           modelData,
            ...
            sourceCluster:   this.cluster,

            cb:              (result) => {
              if (result) {
                set(this, 'confirmIgnoreNameConflicts', true);
                resolve(targetData);
              } else {
                set(this, 'confirmIgnoreNameConflicts', false);
                reject();
              }
            }
          });
