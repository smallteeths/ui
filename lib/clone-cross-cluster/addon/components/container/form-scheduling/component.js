...
export default Component.extend({
  ...
  init() {
    ...
    if ( get(this, 'initialHostId') ) {
      ...
      if (!this.hostChoices.findBy('id', get(this, 'initialHostId'))) {
        this.setErrors([this.intl.t('cloneCrossCluster.newContainer.errors.hostRequired')]);
      }
    }
  },
  ...
  requestedHostIdDidChange: observer('requestedHostId', 'hostChoices.[]', function() {
    ...
    if (hostId && !this.hostChoices.findBy('id', hostId)) {
      this.setErrors([this.intl.t('cloneCrossCluster.newContainer.errors.hostRequired')]);
  ...
  hostChoices: computed('_allNodes.@each.{clusterId,id,name,state}', 'clusterId', function() {
    const list = get(this, '_allNodes').filter((node) => !get(node, 'isUnschedulable'))
      .filterBy('clusterId', get(this, 'clusterId'))
      .map((host) => {
