...
export default Component.extend({
  ...
  vlansubnetdisabledStyle: computed('supportVlansubnet', function() {
  ...
  loadVlansubnets() {
    ...
    if (clusterId && projectId && this.supportVlansubnet) {
      get(this, 'vlansubnet').fetchVlansubnets(clusterId, { labelSelector: encodeURIComponent(`project in (${ projectId.replace(/[:]/g, '-') }, )`) }).then((resp) => {
  ...
  initStaticPod() {
    ...
    set(this, 'staticPod', this.supportVlansubnet && this.macvlanSubnetEnabled)
    this.loadVlansubnets();
    this.validate();
  },
  validate() {
    if (this.supportVlansubnet && this.staticPod && !this.staticPodForm.subnet) {
      this.setErrors([this.intl.t('cloneCrossCluster.newContainer.errors.vlansubnetRequired')]);
    } else {
      this.setErrors([]);
    }
  }
