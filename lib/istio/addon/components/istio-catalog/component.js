...
const WORKLOADS_WITHOUT_MIXER = ['tracing', 'gateway', 'pilot']
...
const MIXER_BREAKING_VERSION = '1.8.0';

const GATEWAY_CUSTOM_ANSWER_DEFAULT = {
  'gateways.istio-ingressgateway.ports[0].targetPort': 8080,
  'gateways.istio-ingressgateway.ports[1].targetPort': 8443,
  'gateways.istio-ingressgateway.ports[2].name':       'status-port',
  'gateways.istio-ingressgateway.ports[2].port':       15021,
  'gateways.istio-ingressgateway.ports[2].targetPort': 15021,
  'gateways.istio-ingressgateway.ports[3].name':       'tls',
  'gateways.istio-ingressgateway.ports[3].port':       15443,
  'gateways.istio-ingressgateway.ports[3].targetPort': 15443,
}

const NOT_SUPPORTED_ANSWER_KEYS = [
  'certmanager.enabled',
  'enableCRDs',
  'galley.enabled',
  'istio_cni.enabled',
  ...
  'mixer.policy.resources.limits.cpu',
  'mixer.policy.resources.limits.memory',
  'mixer.policy.resources.requests.cpu',
  'mixer.policy.resources.requests.memory',
...
export default Component.extend(CrudCatalog, ReservationCheck, CatalogUpgrade, {
  ...
  actions: {
    save(cb) {
      ...
      if (!get(this, 'showMixer')) {
        NOT_SUPPORTED_ANSWER_KEYS.map((key) => {
          delete answers[key]
        })

        if (get(this, 'config.gatewayEnabled')) {
          if (get(this, 'config.gatewayType') === 'NodePort') {
            Object.keys(GATEWAY_CUSTOM_ANSWER_DEFAULT).map((key) => {
              if (!answers[key]) {
                answers[key] = GATEWAY_CUSTOM_ANSWER_DEFAULT[key]
              }
            })
          } else if (get(this, 'config.gatewayType') === 'LoadBalancer') {
            const obj = {
              ...GATEWAY_CUSTOM_ANSWER_DEFAULT,
              'gateways.istio-ingressgateway.ports[0].name': 'http2',
              'gateways.istio-ingressgateway.ports[0].port': 80,
              'gateways.istio-ingressgateway.ports[1].name': 'https',
              'gateways.istio-ingressgateway.ports[1].port': 443,
  ...
  workloadEnabledChange: observer('config.tracingEnabled', 'config.gatewayEnabled', 'config.mixerPolicyEnabled', 'showMixer', function() {
  ...
  showMixerChange: observer('showMixer', 'templateVersion', 'enabled', function() {
    const {
      showMixer, customAnswers = {}, initToggle, templateVersion, enabled
    } = this

    if (!enabled && templateVersion) {
      this.initWorkloads()

      if (!showMixer) {
        NOT_SUPPORTED_ANSWER_KEYS.map((key) => {
          delete customAnswers[key]
        })
      } else {
        Object.keys(HIDDEN_KEYS).map((key) => {
          customAnswers[key] = HIDDEN_KEYS[key]
        })
      }

      set(this, 'customAnswers', customAnswers)

      next(() => set(this, 'initToggle', !initToggle))
    }
  }),
  ...
  saveDisabled: computed('enabled', 'gatewayWarning', 'istioWarning', 'mixerBreaking', 'mixerPolicyWarning', 'mixerTelemetryWarning', 'mixerWarning', 'pilotWarning', 'tracingWarning', 'workloadComponents', function() {
    return [...get(this, 'workloadComponents'), 'mixer'].reduce((out, w) => {
    ...
    }, false) || (get(this, 'istioWarning') || false) || get(this, 'mixerBreaking')
  }),
  ...
  requestsCpu: computed('config.mixerTelemetryRequestCpu', 'config.pilotRequestCpu', 'config.gatewayRequestCpu', 'config.tracingRequestCpu', 'config.mixerPolicyRequestCpu', 'showMixer', function() {
    return WORKLOADS
      .filter((w) => {
        let out = true
        ...
        if (['mixerTelemetry', 'mixerPolicy'].includes(w) && !get(this, 'showMixer')) {
          out = false
        }
  ...
  requestsMemory: computed('config.mixerTelemetryRequestMemory', 'config.pilotRequestMemory', 'config.gatewayRequestMemory', 'config.tracingRequestMemory', 'config.mixerPolicyRequestMemory', 'showMixer', function() {
    return WORKLOADS
      .filter((w) => {
        let out = true
  ...
  insufficientMixerCpu: computed('mixerSchedulableNodes.@each.{allocatable,requested}', 'config.mixerPolicyEnabled', 'config.mixerTelemetryRequestCpu', 'config.mixerPolicyRequestCpu', 'cluster.nodes.@each.{allocatable,requested}', 'showMixer', function() {
    let reservation

    if (get(this, 'config.mixerPolicyEnabled') && get(this, 'showMixer')) {
  ...
  insufficientMixerMemory: computed('mixerSchedulableNodes.@each.{allocatable,requested}', 'config.mixerPolicyEnabled', 'config.mixerTelemetryRequestMemory', 'config.mixerPolicyRequestMemory', 'cluster.nodes.@each.{allocatable,requested}', 'showMixer', function() {
    let reservation
  ...
  mixerWarning: computed('config.{mixerPolicyEnabled,mixerPolicyRequestCpu,mixerPolicyRequestMemory,mixerTelemetryRequestCpu,mixerTelemetryRequestMemory}', 'insufficientMixerCpu', 'insufficientMixerMemory', 'insufficientMixerTelemetryCpu', 'insufficientMixerTelemetryMemory', 'mixerNodeSelectors.[]', 'showMixer', function() {
  ...
  showMixer: computed('templateVersion', function() {
    const templateVersion = get(this, 'templateVersion')

    if (!isEmpty(templateVersion) && Semver.gte(templateVersion, MIXER_BREAKING_VERSION) ) {
      return false
    } else {
      return true
    }
  ...
  workloadComponents: computed('showMixer', function() {
    if (get(this, 'showMixer')) {
      return ['tracing', 'gateway', 'pilot']
    } else {
      return ['mixerTelemetry', 'tracing', 'gateway', 'pilot', 'mixerPolicy']
    }
  }),

  mixerBreaking: computed('templateVersion', 'currentVersion', 'upgradeAvailable', 'enabled', function() {
    const {
      templateVersion, currentVersion, upgradeAvailable, enabled
    } = this

    if (enabled && upgradeAvailable && Semver.gte(templateVersion, MIXER_BREAKING_VERSION) && Semver.lt(currentVersion, MIXER_BREAKING_VERSION)) {
      return true
    } else {
      return false
    }
  ...
  initWorkloads() {
    const workloads = get(this, 'showMixer') ? WORKLOADS : WORKLOADS_WITHOUT_MIXER;

    (['mixerTelemetry', 'mixerPolicy']).map((w) => {
  ...
  enableMonitoring() {
    ...
    answers['exporter-fluentd.enabled'] = 'true'
    answers['exporter-gpu-node.enabled'] = 'false'
    answers['exporter-kube-state.enabled'] = 'true'
    answers['exporter-kubelets.enabled'] = 'true'
    answers['exporter-kubernetes.enabled'] = 'true'
